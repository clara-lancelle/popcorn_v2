...
function MovieGallery({ movies, genre }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 md:m-10">
        {movies?.map((movie, index) => (
          <div key={movie.id} className="overflow-visible h-[28em] lg:h-[26em]">
            <div className="flex flex-col h-full shadow-lg max-w-xs bg-white border border-gray-200 rounded-lg">
              <img
                className="rounded-t-lg mx-auto mt-1"
              ...
              <div className="flex flex-col h-full justify-end p-5 overflow-visible">
                ...
                <div className="flex place-content-between gap-2 lg:flex-row items-center flex-col">
                  <a
                    data-popover-target="popover-overview"
                    onClick={() => setShowModal(movie.id)}
                    className="text-white bg-green-400 hover:cursor-pointer hover:bg-green-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center "
                  >
                    Overview
                  </a>
                  <Link
